 ...
 * </a>
 * */
object WiggleSubsequence2 extends App {
  def wiggleMaxLength(nums: Array[Int]): Int = {

    @scala.annotation.tailrec
    def helper(i: Int = 1, last: Int = nums.head, state: Option[Int] = None, len: Int = 1): Int =
      if (i == nums.length) len
      else nums(i) match {
        case now => if (now == last) helper(i + 1, now, state, len)
        else if (state.isEmpty || state.get > 0 && now > last || state.get < 0 && now < last)
          helper(i + 1, now, Some(last - now), len + 1)
        else helper(i + 1, now, Some(last - now), len)
      }


    if (nums.length == 1) 1 else helper()
...
//Runtime: 658 ms, faster than 78.26% of Scala online submissions for Wiggle Subsequence.
//Memory Usage: 66.5 MB, less than 39.13% of Scala online submissions for Wiggle Subsequence.