...
object WiggleSubsequence extends App {
  def wiggleMaxLength(nums: Array[Int]): Int = {
    ...
    def helper(i: Int = 1, last: Int = nums.head, state: Option[Int] = None, len: Int = 1): Int =
      ...
      else if (nums(i) == last) helper(i + 1, nums(i), state, len)
      else if (state.isEmpty || state.get > 0 && nums(i) > last || state.get < 0 && nums(i) < last)
        helper(i + 1, nums(i), Some(last - nums(i)), len + 1)
      else helper(i + 1, nums(i), Some(last - nums(i)), len)
...
//Runtime: 722 ms, faster than 52.17% of Scala online submissions for Wiggle Subsequence.
//Memory Usage: 66.5 MB, less than 52.17% of Scala online submissions for Wiggle Subsequence.