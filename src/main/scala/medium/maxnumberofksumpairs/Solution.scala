  ...
  def maxOperations(nums: Array[Int], k: Int, pos: Int = 0, m: Map[Int, Int] = Map.empty, count: Int = 0): Int = if (pos >= nums.length) count else nums(pos) match {
    case n => m get (k - n) match {
      case None => m get n match {
        case None => maxOperations(nums, k, pos + 1, m + (n -> 1), count)
        case Some(o) => maxOperations(nums, k, pos + 1, m + (n -> (o + 1)), count)
      }
...
//Runtime: 2217 ms, faster than 100.00% of Scala online submissions for Max Number of K-Sum Pairs.
//Memory Usage: 136.2 MB, less than 100.00% of Scala online submissions for Max Number of K-Sum Pairs.