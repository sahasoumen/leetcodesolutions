...
object Solution3 extends App {
  ...
  def letterCombinations(digits: String): List[String] = if (digits.isEmpty) List() else digits.tail.iterator.foldLeft(map(digits.head - '0').map(_.toString))((list, digit) => map(digit - '0').flatMap(d => list.map(s => s"$s$d")))
