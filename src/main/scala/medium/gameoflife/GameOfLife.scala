  ...
  def gameOfLife(board: Array[Array[Int]]): Unit = (board.length, board(0).length) match {
    case (m, n) =>
      ...
      (for {
        i <- 0 until m
        j <- 0 until n
      } yield board(i)(j) match {
        case 0 => if (neighbors(i, j) == 3) (i, j, 1) else (i, j, 0)
        case 1 => neighbors(i, j) match {
          case 2 | 3 => (i, j, 1)
          case _ => (i, j, 0)
        }
...
//Runtime: 512 ms, faster than 100.00% of Scala online submissions for Game of Life.
